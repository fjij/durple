...
describe("Official durple sub contract", function () {
  let contract;
  let owner;
  let addr1;

  beforeEach(async function () {
    const factory = await ethers.getContractFactory("OfficialDurpleSub");
  ...
  describe("Posts", function() {
    ...
    it("Should not let someone other than owner to post", async function() {
      await expect(contract.connect(addr1).makePost("hash")).to.be.revertedWith("Only the owner can post on this SubDurple.");
    });
