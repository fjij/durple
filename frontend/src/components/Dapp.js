...
export function Dapp () {
  ...
  function startPollingData() {
    pollDataIntervalRef.current = setInterval(() => updateBalance(), 1000);
  ...
  async function getTokenData() {
    const name = await tokenRef.current.name();
    const symbol = await tokenRef.current.symbol();

    setTokenData({name, symbol});
  }

  async function updateBalance() {
    const balance = await tokenRef.current.balanceOf(selectedAddress);
    setBalance(balance);
  }
  ...
  async function transferTokens(to, amount) {
    ...
    try {
      ...
      const tx = await tokenRef.current.transfer(to, amount);
